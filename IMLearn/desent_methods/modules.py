...
class L2(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        """
        return self.weights @ self.weights.T
...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        m = X.shape[0]
        Xi_w = X @ self.weights
        return -1/m * np.sum(y*Xi_w + np.log(1 + np.exp(Xi_w)))
 ...
 m = X.shape[0]        y: ndarray of shape(n_samples,)
            Binary labels of samples to use when computing objective
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        fidelity = self.fidelity_module_#(weights = self.weights)
        regularization = self.regularization_module_#(weights = self.weights)
        return fidelity.compute_output() + self.lam_ * regularization.compute_output()
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        fidelity = self.fidelity_module_#(weights=self.weights)
        regularization = self.regularization_module_#(weights=self.weights)
        return fidelity.compute_jacobian() + self.lam_ * regularization.compute_jacobian()
